...
<xsl:stylesheet version="1.0" 
	...
	<xsl:output method="html" encoding="utf-8" omit-xml-declation="yes" indent="yes" />
	...
	<xsl:template match="/">
		<!-- <p>
			...
			<xsl:value-of select="$todaysdate"/><br/>
			<xsl:value-of select="translate($todaysdate,'-','')"/>
		</p> -->

		<ul class="post-list">
			<xsl:apply-templates select="atom:feed/atom:entry[translate(substring-before(atom:published,'T'),'-','') &lt;= translate($todaysdate,'-','')]" />
		</ul>
	</xsl:template>
